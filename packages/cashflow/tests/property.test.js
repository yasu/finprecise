...
describe("Property-based tests", () => {
    it("PV and FV are inverses (round-trip)", () => {
        fc.assert(fc.property(fc.double({ min: 0.001, max: 0.2, noNaN: true }), // rate
    ...
    it("PMT * NPER + interest ≈ principal for zero-rate", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 360 }), // nper
    ...
    it("IRR of solved cashflows returns rate close to zero NPV", () => {
        fc.assert(fc.property(fc.double({ min: 0.01, max: 0.5, noNaN: true }), // target rate
        fc.array(fc.double({ min: 10, max: 1000, noNaN: true }), { minLength: 2, maxLength: 8 }), (targetRate, positiveFlows) => {
            ...
            if (!result.ok)
                return true; // Skip non-convergent cases
            // Verify NPV at IRR ≈ 0
